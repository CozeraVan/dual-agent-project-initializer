...
TEMPLATE_DIR = ROOT / "src" / "dual_agent_initializer" / "templates"
DIST_DIR = ROOT / "dist"
DIST_FILE = DIST_DIR / "init-agent-project.py"
...
def main() -> int:
    ...
    for p in sorted(TEMPLATE_DIR.rglob("*")):
        if p.is_file():
            templates[p.relative_to(TEMPLATE_DIR).as_posix()] = p.read_text(encoding="utf-8")
    DIST_DIR.mkdir(parents=True, exist_ok=True)
    body = RUNTIME.replace("__TEMPLATES__", pformat(templates, width=100))
    DIST_FILE.write_text(body, encoding="utf-8", newline="\n")
    DIST_FILE.chmod(0o755)
